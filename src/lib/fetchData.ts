...
async function fetchPaginatedPostsWithLimit(
  query: string,
  variables: FetchPostsVariables
): Promise<PostsResponse> {
  let allPosts: Post[] = [];
  ...
  while (hasNextPage && postsCount < MAX_POSTS) {
    const paginatedVariables = { ...variables, after: afterCursor}; 
...
export async function fetchPostsWithLimit(): Promise<PostsResponse> {
  return fetchPaginatedPostsWithLimit(GET_POSTS, {
    slug: "",
    first: 100
  });
}
...
export async function fetchPostsByCategory(categorySlug: string): Promise<PostsResponse> {
